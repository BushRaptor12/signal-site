...
import { createInterestFollow, getAccountUserId, getFollowedInterests } from "@/app/lib/account.server";
...
export async function GET() {
  try {
    ...
    const interests = await getFollowedInterests(userId);
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ interest, ok: true });
  } catch (error) {
